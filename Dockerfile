...
RUN apt install -y burpsuite nmap net-tools metasploit-framework nmap aircrack-ng hydra john hashcat sqlmap recon-ng sublist3r dirb libc6 libgcc-s1 libnode109 libstdc++6 lsof npm nodejs nano
RUN mkdir /root/juice2
RUN git clone https://github.com/juice-shop/juice-shop.git --depth 1 /root/juice2
WORKDIR /root/juice2
RUN npm install
WORKDIR /root
